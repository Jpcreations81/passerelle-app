...
function RechercheGestionnairePaieInline({ value, gestionnaires, onSelect, onCreate, onUpdate }) {
  ...
  return (
    <select className="form-control" value=""
      ...
      style={{ width:'100%', padding:'10px 12px', border:'1.5px solid #dde3f0', borderRadius:8, fontSize:13, fontFamily:'Sora,sans-serif', boxSizing:'border-box' }}>
      <option value="">— Sélectionner un gestionnaire paie —</option>
      {gestionnaires.map(g => (
        <option key={g.id} value={g.id}>{g.prenom} {g.nom}</option>
      ))}
      <option value="__nouveau__">➕ Nouveau gestionnaire paie...</option>
    </select>
  )
}
            ...
            <>
              ...
              <SectionCard icon="📃" title="Contrat de travail">
                <FG cols={3}>
                  ...
                  <div style={{display:'flex',flexDirection:'column',gap:5}}>
                    <label style={{fontSize:11,fontWeight:600,color:'#5a6478',textTransform:'uppercase',letterSpacing:'.4px'}}>Secteur</label>
                    {(isEncadrant && editMode) ? (
                      <select className="form-control" value={v('secteur')||''} onChange={e=>F('secteur')(e.target.value)}>
                      ...
                      <div style={{padding:'10px 12px',background:'#eef1f8',borderRadius:8,fontSize:13,color:'#1a4b8f',fontWeight:600}}>
                        {v('secteur')||'—'}
                        {!isEncadrant && v('secteur') && <span style={{fontSize:11,color:'#9aa3b8',marginLeft:8,fontWeight:400}}>(modifiable par l'encadrant)</span>}
                      </div>
                    )}
                  </div>
                </FG>
