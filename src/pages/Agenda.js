...
export default function Agenda({ profile }) {
  ...
  async function parsePDFModif(file) {
    ...
    try {
      ...
      // On envoie les VM et relais des 12 derniers mois
  ...
  async function saveEvtsModif() {
    ...
    if (erreurs === 0) {
      showToast(`✅ ${toUpdate.length} modifié${toUpdate.length > 1 ? 's' : ''} · ${toCreate.length} créé${toCreate.length > 1 ? 's' : ''} !`)
              ...
              <>
                ...
                <div style={{ maxHeight:420, overflowY:'auto', marginBottom:14 }}>
                  {evtsModif.map((evt, i) => {
                    ...
                    const actionColor = isModif ? '#d97706' : isCreate ? '#2e8b4a' : '#9aa3b8'
                    const actionBg = isModif ? '#fef3e2' : isCreate ? '#e6f5eb' : '#eef1f8'
                    const actionLabel = isModif ? '🔄 Modifier' : isCreate ? '➕ Créer' : '✓ Inchangé'
