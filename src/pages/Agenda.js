        ...
        for (const evt of data.evenements) {
          ...
          const extractNomRelais = (evt) => {
            ...
            if (evt.lieu && evt.categorie === 'relais') {
              // Ex: "Famille d'accueil relais Monsieur ABOUDAOUD Fares" → "ABOUDAOUD Fares"
              // Ex: "Chez Monsieur ABOUDAOUD Fares" → "ABOUDAOUD Fares"
              const m = evt.lieu.match(/(?:chez\s+(?:monsieur|madame|m\.|mme\.?)\s+|famille\s+(?:d['']accueil\s+)?relais\s+(?:monsieur|madame|m\.|mme\.?)?\s*)([A-Z][A-Z]+(?:\s+[A-Za-zÀ-ÿ]+)?)/i)
