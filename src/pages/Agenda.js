...
export default function Agenda({ profile }) {
  ...
  async function parsePDF(file) {
    ...
    try {
      ...
      if (data.evenements && data.evenements.length > 0) {
        // Mapper enfants_noms → enfant_ids en cherchant dans le state enfants
        const evtsAvecIds = data.evenements.map(evt => {
          const ids = []
          if (evt.enfants_noms && evt.enfants_noms.length > 0) {
            evt.enfants_noms.forEach(nom => {
              // Cherche par prénom ou nom complet (insensible à la casse)
              const nomLower = nom.toLowerCase()
              const enf = enfants.find(e =>
                nomLower.includes(e.prenom.toLowerCase()) ||
                nomLower.includes(e.nom.toLowerCase()) ||
                `${e.prenom} ${e.nom}`.toLowerCase() === nomLower
              )
              if (enf && !ids.includes(enf.id)) ids.push(enf.id)
            })
          }
          // Construire les notes avec infos TISF si présentes
          let notes = evt.notes || ''
          if (evt.tisf_debut && evt.tisf_fin) {
            const tisfInfo = `TISF : ${evt.tisf_debut} → ${evt.tisf_fin}`
            notes = notes ? `${notes} · ${tisfInfo}` : tisfInfo
          } else if (evt.tisf_debut || evt.tisf_fin) {
            const tisfInfo = `TISF présente`
            notes = notes ? `${notes} · ${tisfInfo}` : tisfInfo
          }
          return { ...evt, enfant_ids: ids, notes }
        })
        setEvtsImportes(evtsAvecIds)
  ...
  async function saveEvtsImportes(pdfDocumentId) {
    const selectionnes = evtsImportes.filter((_, i) => evtsImportesChecked[i])
    if (selectionnes.length === 0) { showToast('⚠️ Aucun événement sélectionné'); return }

    const rows = []
    selectionnes.forEach(evt => {
      const enfantsACree = evt.enfant_ids && evt.enfant_ids.length > 0 ? evt.enfant_ids : [null]
                    ...
                    return (
                      <div key={i} style={{ display:'flex', gap:10, padding:'10px 12px', marginBottom:8, borderRadius:9, border:`2px solid ${checked ? cat.color : '#dde3f0'}`, background: checked ? cat.bg : '#f8f9fb', transition:'all .15s' }}>
                        ...
                        <div style={{ flex:1 }}>
                          ...
                          {/* Enfants détectés */}
                          {evt.enfant_ids && evt.enfant_ids.length > 0 && (
                            <div style={{ marginTop:5, display:'flex', gap:4, flexWrap:'wrap' }}>
                              {evt.enfant_ids.map(eid => {
                                const enf = enfants.find(e => e.id === eid)
                                return enf ? (
                                  <span key={eid} style={{ fontSize:10, padding:'2px 8px', borderRadius:10, background: couleursEnfants[eid] || '#1a4b8f', color:'#fff', fontWeight:600 }}>
                                    {enf.prenom} {enf.nom}
                                  </span>
                                ) : null
                              })}
                              <span style={{ fontSize:10, color:'#9aa3b8', fontStyle:'italic' }}>→ 1 événement par enfant</span>
                            </div>
                          )}
                        </div>
                      </div>
