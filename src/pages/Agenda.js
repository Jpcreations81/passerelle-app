...
export default function Agenda({ profile }) {
  ...
  const [newEvt, setNewEvt] = useState({
    ...
    enfantsSelectionnes: [],
    relais_type: 'af',
    relais_structure_id: null,
    relais_nom_libre: '',
    vm_presents: [],
    complement_titre: ''
  })
  // États pour la gestion des structures relais
  const [relaisStructures, setRelaisStructures] = useState([]) // structures en base
  const [showRelaisModal, setShowRelaisModal] = useState(false) // modal création structure
  ...
  const [pdfModifParsing, setPdfModifParsing] = useState(false)
  ...
  const [evtsModif, setEvtsModif] = useState([]) // événements à modifier
  const [evtsModifChecked, setEvtsModifChecked] = useState({}) // { idx: bool }
  const [resumeModif, setResumeModif] = useState('') // résumé des changements
  ...
  // Charger toutes les structures relais connues
  const fetchRelaisStructures = useCallback(async () => {
    const { data } = await supabase
      .from('relais_structures')
      .select('*')
  ...
  async function saveRelaisStructure() {
    if (!newRelais.nom) { showToast('⚠️ Nom requis'); return }
    const { data, error } = await supabase.from('relais_structures').insert({
      nom: newRelais.nom,
      type: newRelais.type,
      adresse: newRelais.adresse || null,
      telephone: newRelais.telephone || null,
      email: newRelais.email || null,
      notes: newRelais.notes || null,
      territoire: profile?.territoire || null,
      created_by: profile.id
    }).select().single()
    if (!error && data) {
      showToast('✅ Structure relais créée !')
      setRelaisStructures(prev => [...prev, data])
      // Sélectionner automatiquement cette structure
      setNewEvt(n => ({ ...n, relais_structure_id: data.id, relais_nom_libre: data.nom }))
      setShowRelaisModal(false)
      setNewRelais({ nom:'', type:'tiers', adresse:'', telephone:'', email:'', notes:'' })
  ...
  async function saveEdit() {
    ...
    // Si l'événement fait partie d'une série → demander
    if (selectedEvt?.serie_id) {
      setEditSerieAction(() => (modifierSerie) => doSaveEdit(modifierSerie))
  ...
  async function doSaveEdit(modifierSerie) {
    ...
    setShowSerieModal(false)
      ...
      const updateData = {        titre: editEvt.titre, categorie: editEvt.categorie,        date_debut: debut.toISOString(), date_fin: fin.toISOString(),        lieu: editEvt.lieu, notes: editEvt.notes,      }      if (modifierSerie && selectedEvt.serie_id) {        // Modifier tous les événements de la série — seulement titre, lieu, notes, heures (pas les dates)        const { data: serieEvts } = await supabase          .from('evenements')          .select('id, date_debut, date_fin')          .eq('serie_id', selectedEvt.serie_id)        if (serieEvts) {          for (const evt of serieEvts) {            const evtDebut = new Date(evt.date_debut)            const evtFin = new Date(evt.date_fin)            evtDebut.setHours(debut.getHours(), debut.getMinutes())            evtFin.setHours(fin.getHours(), fin.getMinutes())            await supabase.from('evenements').update({              titre: editEvt.titre,              lieu: editEvt.lieu,              notes: editEvt.notes,              date_debut: evtDebut.toISOString(),              date_fin: evtFin.toISOString(),            }).eq('id', evt.id)          }          showToast(`✅ ${serieEvts.length} événements de la série modifiés !`)        }      } else {        const { error } = await supabase.from('evenements').update(updateData).eq('id', selectedEvt.id)        if (!error) showToast('✅ Événement modifié !')        else { showToast('❌ Erreur'); return }      }      setEditMode(false); setShowDetailModal(false); fetchEvenements()    }    setShowSerieModal(false)  }
  ...
  const relaisStructuresMap = useMemo(() => {
    const m = {}
    relaisStructures.forEach(s => { m[s.id] = s })
    return m
  }, [relaisStructures])
        ...
        if (evt.enfant_ids && evt.enfant_ids.length > 0) {
          evt.enfant_ids.forEach((enfantId, idx) => {
            ...
            if (evt.categorie === 'relais') {
              ...
              // Enrichir le titre avec la structure relais si disponible
              if (evt.categorie === 'relais') {
                const RELAIS_LABELS = {
                  af: 'Relais fam.', parrain: 'Relais parrain', tiers: 'Relais tiers',
                  lva: 'Relais LVA', foyer: 'Relais foyer', centre_loisirs: 'Relais centre',
                  base_dep: 'Relais base dép.', colonie: 'Relais colonie', autre: 'Relais'
                }
                const typeLabel = RELAIS_LABELS[evt.relais_type] || 'Relais'
                if (evt.relais_structure_id && relaisStructuresMap[evt.relais_structure_id]) {
                  const struct = relaisStructuresMap[evt.relais_structure_id]
                  titrePOV = (enfantPrenom ? enfantPrenom + ' — ' : '') + typeLabel + ' ' + struct.nom
                } else if (evt.relais_nom_libre) {
                  titrePOV = (enfantPrenom ? enfantPrenom + ' — ' : '') + typeLabel + ' ' + evt.relais_nom_libre
                } else if (evt.notes && evt.notes.toLowerCase().includes('inconnu')) {
                  titrePOV = (enfantPrenom ? enfantPrenom + ' — ' : '') + 'Relais fam. ???'
                }
              }
            } else {
  ...
  }, [evenements, enfants, couleursEnfants, afProfiles, filtres, profile, relaisStructuresMap])
  ...
  async function saveEvt() {
    ...
    // Ajouter serie_id aux rows principaux si série
    const rowsAvecSerie = serieId
      ? rows.map(r => ({ ...r, serie_id: serieId }))
      : rows

    // Créer aussi les événements pour les dates supplémentaires
    const rowsSup = []
    if (newEvt.dates_sup && newEvt.dates_sup.length > 0) {
      newEvt.dates_sup.forEach(dateSup => {
        const debutSup = new Date(`${dateSup}T${newEvt.heure_debut}:00`)
        const finSup = new Date(`${dateSup}T${newEvt.heure_fin}:00`)
        enfantsACree.forEach(enfantId => {
          rowsSup.push({
            ...rows[0],
            serie_id: serieId,
            date_debut: debutSup.toISOString(),
            date_fin: finSup.toISOString(),
            ...(enfantId ? { enfant_ids: [enfantId] } : { enfant_ids: [] })
          })
        })
      })
    }
    ...
    try {
      ...
      if (data.evenements && data.evenements.length > 0) {
        ...
        for (const evt of data.evenements) {
          ...
          // Extraire le nom de la famille relais depuis relais_nom ou depuis le lieu
          const extractNomRelais = (evt) => {
            if (evt.relais_nom) return evt.relais_nom
            if (evt.lieu && evt.categorie === 'relais') {
              const exclusions = ['FAMILLE', 'ACCUEIL', 'RELAIS', 'MONSIEUR', 'MADAME', 'CHEZ', 'MME', 'MR']
              const mots = evt.lieu.split(' ')
              const nomMots = []
              for (const mot of mots) {
                const clean = mot.replace(/[^A-Za-z]/g, '')
                const isAllCaps = clean.length >= 3 && clean === clean.toUpperCase() && !exclusions.includes(clean.toUpperCase())
                const isCapitalized = nomMots.length > 0 && clean.length > 0 && clean[0] === clean[0].toUpperCase() && clean[0] !== clean[0].toLowerCase()
                if (isAllCaps) { nomMots.push(mot.replace(/[,.]$/,'')) }
                else if (isCapitalized) { nomMots.push(mot.replace(/[,.]$/,'')); break }
                else if (nomMots.length > 0) { break }
              }
              if (nomMots.length > 0) return nomMots.join(' ')
            }
            return null
          }
          const relaisNomBrut = extractNomRelais(evt)
          console.log('Evt relais check:', evt.titre, '| categorie:', evt.categorie, '| relais_nom:', evt.relais_nom)
          if (relaisNomBrut && evt.categorie === 'relais') {
            const relaisNomLower = relaisNomBrut.toLowerCase().trim()
            ...
            if (relaisProfile) {
              ...
              if (nomParts.length > 0) {
                try {
                  const { data: foundProfiles, error: errSearch } = await supabase
                    .from('profiles')
                    .select('id, nom, prenom')
                    .eq('role', 'af')
                    .ilike('nom', `%${nomParts[0]}%`)
                  console.log('Recherche relais:', nomParts[0], '→ résultats:', foundProfiles, 'erreur:', errSearch)
                  if (!errSearch && foundProfiles && foundProfiles.length > 0) {
                    participantsIds = [foundProfiles[0].id]
                    relaisLabel = `${foundProfiles[0].prenom} ${foundProfiles[0].nom}`
                    foundInDB = true
                  }
                } catch(e) {
                  console.log('Recherche relais DB exception:', e.message)
                }
              }
              console.log('foundInDB:', foundInDB, '| relais_nom:', evt.relais_nom)
              if (!foundInDB) {
                // Introuvable → garder en note pour liaison manuelle
                relaisLabel = evt.relais_nom
                const nomRelaisAffiche = evt.relais_nom && evt.relais_nom !== 'undefined' ? evt.relais_nom : 'inconnu'
                notes = notes
                  ? `${notes} · ⚠️ Famille relais à lier : ${nomRelaisAffiche}`
                  : `⚠️ Famille relais à lier : ${nomRelaisAffiche}`
                console.log('Note ajoutée:', notes)
              }
            }
          }
        ...
        // Initialiser toutes les coches à true explicitement
        const checked = {}
  ...
  async function saveEvtsImportes(pdfDocumentId) {
    ...
    const toParisISO = (isoStr) => {
      ...
      // Si la date n'a pas de fuseau (ex: "2026-03-30T15:00:00"), 
      // on la traite comme heure Paris et on convertit en UTC pour Supabase
      if (!isoStr.includes('+') && !isoStr.endsWith('Z')) {
        // Créer la date en forçant le fuseau Paris
        const [datePart, timePart] = isoStr.split('T')
        const [y, m, d] = datePart.split('-').map(Number)
        const [h, min, s] = (timePart || '00:00:00').split(':').map(Number)
        // Utiliser Intl pour déterminer l'offset Paris à cette date
        ...
        const offsetHours = parisOffset.includes('+') 
          ? -parseInt(parisOffset.split('+')[1]) 
          : parseInt(parisOffset.split('-')[1])
  ...
  function buildTitreAuto(evt, enfantsList, couleursMap) {
    ...
    let complement = ''
    if (evt.categorie === 'vm' && evt.vm_presents?.length > 0) {
      const LABELS_VM = {
        pere: 'Père', mere: 'Mère', parents: 'Parents',
        fratrie: 'Fratrie', pere_fratrie: 'Père + Fratrie',
        mere_fratrie: 'Mère + Fratrie', parents_fratrie: 'Parents + Fratrie'
      }
      complement = evt.vm_presents.map(p => LABELS_VM[p] || p).join(' + ')
    } else if (evt.categorie === 'relais' && evt.relais_nom_libre) {
      const LABELS_TYPE = {
        af: 'Fam.', parrain: 'Parrain', tiers: 'Tiers',
        lva: 'LVA', foyer: 'Foyer', centre_loisirs: 'Centre',
        base_dep: 'Base dép.', colonie: 'Colonie', autre: ''
      }
      const typeLabel = LABELS_TYPE[evt.relais_type] || ''
    ...
    // Ajouter le lieu si renseigné et non relais (le lieu du relais est déjà dans complement)
    if (evt.lieu && !['relais'].includes(evt.categorie)) {
      return `${titre} — ${evt.lieu}`
    }
    return titre
  }
  ...
  // ── Import PDF de MODIFICATION ────────────────────────────────────────────
  async function parsePDFModif(file) {
    if (!file || file.type !== 'application/pdf') { showToast('⚠️ Fichier PDF requis'); return }
    setPdfModifParsing(true)
    setEvtsModif([])
    try {
      const base64 = await new Promise((res, rej) => {
        const reader = new FileReader()
      ...
      // Préparer les événements existants pour le contexte Claude
      // On envoie les dates en heure Paris lisible pour que Claude puisse les reconnaître
      ...
      const evtsContexte = evenements
        .filter(e => ['vm','relais','ase','medical'].includes(e.categorie))
        .filter(e => new Date(e.date_debut) > dateRef)
        .map(e => ({
          id: e.id,
          categorie: e.categorie,
          date_debut: e.date_debut,
          date_fin: e.date_fin,
          date_debut_lisible: fmtParis(e.date_debut),
          date_fin_lisible: fmtParis(e.date_fin),
          titre: e.titre,
          enfants_noms: e.enfant_ids
            ? e.enfant_ids.map(id => { const en = enfants.find(x => x.id === id); return en ? en.prenom + ' ' + en.nom : '' }).filter(Boolean)
            : []
        }))

      const resp = await fetch('/api/parse-pdf-modif', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ pdf: base64, filename: file.name, evenements_existants: evtsContexte })
      })
      if (!resp.ok) throw new Error('Erreur serveur ' + resp.status)
      const data = await resp.json()

      if (data.evenements && data.evenements.length > 0) {
        // Mapper enfants_noms → enfant_ids (même logique que parsePDF)
        const evtsAvecIds = []
        for (const evt of data.evenements) {
          const ids = []
          if (evt.enfants_noms && evt.enfants_noms.length > 0) {
            evt.enfants_noms.forEach(nom => {
              const nomLower = nom.toLowerCase().trim()
              const parts = nomLower.split(' ').filter(p => p.length > 1)
              const enf = enfants.find(e => {
                const prenomLower = e.prenom.toLowerCase().trim()
                const nomFamLower = e.nom.toLowerCase().trim()
                return parts.includes(prenomLower) || nomLower.includes(prenomLower)
              })
              if (enf && !ids.includes(enf.id)) ids.push(enf.id)
            })
          }
          ...
          // Trouver l'événement existant à modifier
          let evtExistant = null
          if (evt.action === 'modifier' && evt.evenement_id_existant) {
            evtExistant = evenements.find(e => e.id === evt.evenement_id_existant)
          }
          // Fallback : chercher par catégorie + enfant + date proche
          if (!evtExistant && evt.action === 'modifier' && ids.length > 0) {
            const dateNouv = new Date(evt.date_debut)
            evtExistant = evenements.find(e => {
              if (e.categorie !== evt.categorie) return false
              if (!e.enfant_ids || !e.enfant_ids.some(id => ids.includes(id))) return false
              const diff = Math.abs(new Date(e.date_debut) - dateNouv) / (1000 * 60 * 60 * 24)
              return diff <= 30 // dans les 30 jours
            })
          }
        ...
        setEvtsModif(evtsAvecIds)
        setResumeModif(data.resume || '')
        const checked = {}
        evtsAvecIds.forEach((_, i) => { checked[i] = true })
        setEvtsModifChecked(checked)
      } else {
        showToast('⚠️ Aucun événement détecté dans ce PDF')
      }
    } catch(e) {
      ...
      setPdfModifParsing(false)
    }
  }

  // ── Valider les modifications ─────────────────────────────────────────────
  async function saveEvtsModif() {
    const selectionnes = evtsModif.filter((_, i) => evtsModifChecked[i] === true)
    if (selectionnes.length === 0) { showToast('⚠️ Aucun événement sélectionné'); return }

    const toUpdate = selectionnes.filter(e => e.action === 'modifier' && e._evtExistant)
    const toCreate = selectionnes.filter(e => e.action === 'creer' || (e.action === 'modifier' && !e._evtExistant))
    const toDelete = selectionnes.filter(e => e.action === 'supprimer' && e._evtExistant)

    let erreurs = 0

    // Supprimer les événements remplacés
    for (const evt of toDelete) {
      const { error } = await supabase.from('evenements').delete().eq('id', evt._evtExistant.id)
      if (error) erreurs++
    }
    ...
    // Même fonction de conversion Paris → UTC
    const toParisISOModif = (isoStr) => {
      if (!isoStr) return isoStr
      if (!isoStr.includes('+') && !isoStr.endsWith('Z')) {
        const [datePart, timePart] = isoStr.split('T')
        const [y, m, d] = datePart.split('-').map(Number)
    ...
    for (const evt of toUpdate) {
      const { error } = await supabase.from('evenements').update({
        date_debut: toParisISOModif(evt.date_debut),
        date_fin: toParisISOModif(evt.date_fin),
        lieu: evt.lieu || evt._evtExistant.lieu,
        notes: evt.notes || evt._evtExistant.notes,
        titre: evt.titre || evt._evtExistant.titre,
      }).eq('id', evt._evtExistant.id)
      if (error) erreurs++
    }

    // Créer les nouveaux événements
    if (toCreate.length > 0) {
      const rows = toCreate.map(evt => ({
        titre: evt.titre,
        categorie: evt.categorie || 'vm',
    ...
    if (erreurs === 0) {
      const msgs = []
      if (toDelete.length) msgs.push(`${toDelete.length} supprimé${toDelete.length > 1 ? 's' : ''}`)
      if (toUpdate.length) msgs.push(`${toUpdate.length} modifié${toUpdate.length > 1 ? 's' : ''}`)
      if (toCreate.length) msgs.push(`${toCreate.length} créé${toCreate.length > 1 ? 's' : ''}`)
      showToast('✅ ' + msgs.join(' · ') + ' !')
      ...
      showToast(`⚠️ ${erreurs} erreur(s) — vérifiez la console`)
    }
  }

  async function deleteEvt(id) {
    if (selectedEvt?.serie_id) {
      setEditSerieAction(() => async (modifierSerie) => {
        if (modifierSerie) {
          const { data: serieEvts } = await supabase.from('evenements').select('id').eq('serie_id', selectedEvt.serie_id)
          if (serieEvts) {
            for (const evt of serieEvts) { await supabase.from('evenements').delete().eq('id', evt.id) }
            showToast(`🗑 ${serieEvts.length} événements de la série supprimés`)
          }
        } else {
          await supabase.from('evenements').delete().eq('id', id)
          showToast('🗑 Événement supprimé')
        }
        setShowSerieModal(false); setShowDetailModal(false); fetchEvenements()
      })
      setShowSerieModal(true)
      return
    }
          ...
          {evts.slice(0, 3).map((ev, ei) => {
            ...
            return (
              <div key={ei} onClick={e => openEvt(ev, e)}
                style={{ fontSize:9, padding:'2px 5px', borderRadius:3, marginBottom:2, background: ev._couleur || cat.color, color:'#fff', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis', cursor:'pointer', fontWeight:500, animation: isRelaisInconnu ? 'blink 1.2s step-start infinite' : 'none', outline: isRelaisInconnu ? '1.5px solid #fff' : 'none' }}>
              ...
              {evtsDuJour(d).length > 0 && (
                <div style={{ display:'flex', flexWrap:'wrap', gap:2, marginTop:4, justifyContent:'center' }}>
                  {evtsDuJour(d).map((ev, ei) => {
                    ...
                    const isInconnu = ev.categorie === 'relais' && ev.notes && ev.notes.toLowerCase().includes('inconnu')
                    return <div key={ei} onClick={e => openEvt(ev, e)} style={{ fontSize:9, padding:'2px 6px', borderRadius:3, background:wColor, color:'#fff', cursor:'pointer', fontWeight:500, maxWidth:90, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', animation: isInconnu ? 'blink 1.2s step-start infinite' : 'none' }}>{ev._titrePOV || ev.titre}</div>
                  })}
                </div>
              )}
            </div>
          ...
          evts.map((ev, idx) => {
            ...
            return (
              <div key={`${ev.id}-${idx}`} onClick={e => openEvt(ev, e)}
                style={{ display:'flex', gap:12, padding:'12px 16px', borderBottom:'1px solid #dde3f0', cursor:'pointer', borderLeft:`4px solid ${ev._couleur || cat.color}`, transition:'all .15s', animation: (ev.categorie === 'relais' && ev.notes && ev.notes.toLowerCase().includes('inconnu')) ? 'blink 1.2s step-start infinite' : 'none' }}
    ...
    <div className="app-layout">
      ...
      <div className="main-content">
        ...
        <PageHeader
          icon="📅"
          title="Agenda"
          subtitle={`${profile?.prenom || ''} ${profile?.nom || ''} · ${profile?.territoire || ''}`}
        >
        ...
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal-box" style={{ maxWidth:580 }} onClick={e => e.stopPropagation()}>
            ...
            <div className="form-grid-2">
                ...
                <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                  {Object.entries(CATEGORIES).map(([k, v]) => (
                    <button key={k} type="button"
                      ...
                      style={{ padding:'5px 10px', borderRadius:20, border:`1.5px solid ${newEvt.categorie === k ? v.color : '#dde3f0'}`, background: newEvt.categorie === k ? v.color : '#fff', color: newEvt.categorie === k ? '#fff' : '#5a6478', fontSize:11, fontWeight: newEvt.categorie === k ? 700 : 500, cursor:'pointer', transition:'all .15s' }}>
                      {v.label}
                    </button>
                  ))}
                </div>
                ...
                <div className="form-group col-span-2">
                  ...
                  <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                    {[
                      ...
                      { k:'mere', l:'👩 Mère' },
                      { k:'fratrie', l:'👦 Fratrie' },
                      { k:'pere_fratrie', l:'👨👦 Père + Fratrie' },
                      { k:'mere_fratrie', l:'👩👦 Mère + Fratrie' },
                      { k:'parents_fratrie', l:'👨‍👩‍👧 Parents + Fratrie' },
                    ].map(({ k, l }) => {
                      const sel = newEvt.vm_presents?.includes(k)
                      return (
                        <button key={k} type="button"
                          onClick={() => {
                            const newPresents = sel
                              ? newEvt.vm_presents.filter(p => p !== k)
                              : [k] // 1 seul choix à la fois
                            const updated = { ...newEvt, vm_presents: newPresents }
                            setNewEvt({ ...updated, titre: buildTitreAuto(updated, enfants, couleursEnfants) })
                          }}
                          style={{ padding:'5px 10px', borderRadius:20, border:`1.5px solid ${sel ? '#1a4b8f' : '#dde3f0'}`, background: sel ? '#1a4b8f' : '#fff', color: sel ? '#fff' : '#5a6478', fontSize:11, fontWeight: sel ? 700 : 500, cursor:'pointer', transition:'all .15s' }}>
                          {l}
                        </button>
                      )
                    })}
              ...
              {/* ── 3b. SOUS-TYPE ASE ── */}
              {newEvt.categorie === 'ase' && (
                <div className="form-group col-span-2">
                  <label className="form-label">📋 Type de réunion</label>
                  <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                    {[
                      { k:'Réunion ASE', l:'📋 Réunion' },
                      { k:'Synthèse', l:'📝 Synthèse' },
                      { k:'Audience TJ', l:'⚖️ Audience TJ' },
                      { k:'Convocation', l:'📨 Convocation' },
                    ].map(({ k, l }) => {
                      const sel = newEvt.complement_titre === k
                      return (
                        <button key={k} type="button"
                          onClick={() => {
                            const updated = { ...newEvt, complement_titre: sel ? '' : k }
                            setNewEvt({ ...updated, titre: buildTitreAuto(updated, enfants, couleursEnfants) })
                          }}
                          style={{ padding:'5px 10px', borderRadius:20, border:`1.5px solid ${sel ? '#1a4b8f' : '#dde3f0'}`, background: sel ? '#1a4b8f' : '#fff', color: sel ? '#fff' : '#5a6478', fontSize:11, fontWeight: sel ? 700 : 500, cursor:'pointer' }}>
                          {l}
                        </button>
                      )
                    })}
              ...
              {/* ── 3c. SOUS-TYPE MÉDICAL / SCOLAIRE ── */}
              {['medical', 'scolaire'].includes(newEvt.categorie) && (
                <div className="form-group col-span-2">
                  <label className="form-label">{newEvt.categorie === 'medical' ? '🏥 Spécialiste / Médecin' : '🏫 Objet'}</label>
                  <input className="form-control"
                    value={newEvt.complement_titre}
                    placeholder={newEvt.categorie === 'medical' ? 'Ex: Dr. Martin, Pédiatre...' : 'Ex: Réunion parents, Conseil de classe...'}
                    onChange={e => {
                      const updated = { ...newEvt, complement_titre: e.target.value }
                      setNewEvt({ ...updated, titre: buildTitreAuto(updated, enfants, couleursEnfants) })
                    }}
                  />
                </div>
              ...
              <div className="form-group">
                ...
                <input className="form-control" type="date" value={newEvt.date_debut}
                  onChange={e => setNewEvt(n => ({...n, date_debut: e.target.value, date_fin: n.date_fin || e.target.value}))} />
              </div>
              <div className="form-group">
                <label className="form-label">🕐 Heure début</label>
                <input className="form-control" type="time" value={newEvt.heure_debut}
                  onChange={e => setNewEvt(n => ({...n, heure_debut: e.target.value}))} />
              </div>

              {/* ── DATE FIN + HEURE FIN ── */}
              <div className="form-group">
                <label className="form-label">📅 Date fin</label>
                <input className="form-control" type="date" value={newEvt.date_fin}
                  onChange={e => setNewEvt(n => ({...n, date_fin: e.target.value}))} />
              </div>
              <div className="form-group">
                <label className="form-label">🕐 Heure fin</label>
                <input className="form-control" type="time" value={newEvt.heure_fin}
                  onChange={e => setNewEvt(n => ({...n, heure_fin: e.target.value}))} />
              </div>

              {/* ── DATES SUPPLÉMENTAIRES (duplication) ── */}
              <div className="form-group col-span-2">
                <label className="form-label">
                  📆 Dates supplémentaires
                  <span style={{ fontSize:10, color:'#9aa3b8', fontWeight:400, marginLeft:6 }}>
                    (créera 1 événement identique par date)
                  </span>
                </label>
                <div style={{ display:'flex', flexWrap:'wrap', gap:6, marginBottom:6 }}>
                  {(newEvt.dates_sup || []).map((d, i) => (
                    <div key={i} style={{ display:'flex', alignItems:'center', gap:4, padding:'3px 10px', borderRadius:15, background:'#e8eef8', border:'1px solid #c4d4f5', fontSize:11, fontWeight:600, color:'#1a4b8f' }}>
                      {new Date(d).toLocaleDateString('fr-FR', { weekday:'short', day:'numeric', month:'short' })}
                      <span style={{ cursor:'pointer', color:'#c0392b', marginLeft:3, fontWeight:700 }}
                        onClick={() => setNewEvt(n => ({ ...n, dates_sup: n.dates_sup.filter((_, j) => j !== i) }))}>×</span>
                    </div>
                  ))}
                </div>
              ...
              {newEvt.categorie === 'relais' && (
                <div className="form-group col-span-2" style={{ background:'#e0f2fe', borderRadius:9, padding:12, border:'1px solid #bae6fd' }}>
                  <label className="form-label" style={{ color:'#0891b2' }}>🔄 Type de relais</label>
                  <div style={{ display:'flex', gap:6, flexWrap:'wrap', marginBottom:10 }}>
                    {[
                      { k:'af', l:'👨‍👩‍👧 Famille AF' },
                      { k:'parrain', l:'🤝 Parrain/Marraine' },
                      { k:'tiers', l:'👤 Tiers de confiance' },
                      { k:'lva', l:'🏠 LVA' },
                      { k:'foyer', l:'🏛️ Foyer' },
                      { k:'centre_loisirs', l:'🎨 Centre loisirs' },
                      { k:'base_dep', l:'⛺ Base dép.' },
                      { k:'colonie', l:'🏕️ Colonie' },
                      { k:'autre', l:'📋 Autre' },
                    ].map(({ k, l }) => (
                      <button key={k} type="button"
                        onClick={() => setNewEvt(n => ({ ...n, relais_type: k, relais_structure_id: null, relais_nom_libre: '' }))}
                        style={{ padding:'5px 10px', borderRadius:20, border:`1.5px solid ${newEvt.relais_type === k ? '#0891b2' : '#dde3f0'}`, background: newEvt.relais_type === k ? '#0891b2' : '#fff', color: newEvt.relais_type === k ? '#fff' : '#5a6478', fontSize:11, fontWeight: newEvt.relais_type === k ? 700 : 500, cursor:'pointer', transition:'all .15s' }}>
                        {l}
                      </button>
                    ))}
                  </div>

                  {/* Si AF → afficher relais habituels + recherche */}
                    ...
                    <div>
                      ...
                      {(() => {
                        ...
                        evenements.forEach(evt => {
                          ...
                          if (!evt.enfant_ids?.some(id => enfantsFiltres.includes(id))) return
                  ...
                  {/* Autres types → rechercher dans structures ou créer */}
                  {newEvt.relais_type !== 'af' && (
                    <div>
                      <label className="form-label">Structure / Personne</label>
                      <div style={{ display:'flex', gap:6 }}>
                        <div style={{ flex:1, position:'relative' }}>
                          <input className="form-control"
                            placeholder="Rechercher ou saisir un nom..."
                            value={relaisRecherche || newEvt.relais_nom_libre}
                            onChange={e => {
                              setRelaisRecherche(e.target.value)
                              setNewEvt(n => ({ ...n, relais_nom_libre: e.target.value, relais_structure_id: null }))
                            }}
                          />
                          {/* Suggestions */}
                          {relaisRecherche.length > 1 && (
                            <div style={{ position:'absolute', top:'100%', left:0, right:0, background:'#fff', border:'1px solid #dde3f0', borderRadius:7, zIndex:100, maxHeight:160, overflowY:'auto', boxShadow:'0 4px 12px rgba(0,0,0,.1)' }}>
                              {relaisStructures
                                .filter(s => s.nom.toLowerCase().includes(relaisRecherche.toLowerCase()) && (s.type === newEvt.relais_type || newEvt.relais_type === 'autre'))
                                .map(s => (
                                  <div key={s.id}
                                    onClick={() => { setNewEvt(n => ({ ...n, relais_structure_id: s.id, relais_nom_libre: s.nom, lieu: s.adresse || '' })); setRelaisRecherche('') }}
                                    style={{ padding:'8px 12px', cursor:'pointer', fontSize:12, borderBottom:'1px solid #f0f0f0' }}
                                    onMouseOver={e => e.currentTarget.style.background = '#f4f6fb'}
                                    onMouseOut={e => e.currentTarget.style.background = '#fff'}>
                                    <div style={{ fontWeight:600 }}>{s.nom}</div>
                                    {s.adresse && <div style={{ fontSize:10, color:'#9aa3b8' }}>📍 {s.adresse}</div>}
                                  </div>
                                ))}
                              <div
                                onClick={() => { setShowRelaisModal(true); setRelaisRecherche('') }}
                                style={{ padding:'8px 12px', cursor:'pointer', fontSize:11, color:'#0891b2', fontWeight:700, background:'#f0f9ff' }}>
                                + Créer "{relaisRecherche}" comme nouvelle structure
                              </div>
                            </div>
                          )}
                        </div>
                        <button type="button" className="btn btn-secondary" style={{ fontSize:11, whiteSpace:'nowrap' }}
                          onClick={() => setShowRelaisModal(true)}>
                          + Nouveau
                        </button>
                      </div>
                      {newEvt.relais_structure_id && relaisStructures.find(s => s.id === newEvt.relais_structure_id) && (() => {
                        const s = relaisStructures.find(s => s.id === newEvt.relais_structure_id)
                        return (
                          <div style={{ marginTop:6, padding:'6px 10px', background:'#f0f9ff', borderRadius:7, fontSize:11, border:'1px solid #bae6fd' }}>
                            ✅ <strong>{s.nom}</strong>
              ...
              <div className="form-group col-span-2">
                <label className="form-label">✏️ Titre <span style={{ fontSize:10, color:'#9aa3b8', fontWeight:400 }}>(généré automatiquement, modifiable)</span></label>
                <input className="form-control" value={newEvt.titre}
                  onChange={e => setNewEvt(n => ({...n, titre: e.target.value}))}
                  placeholder="Ex: Ava P. — VM — Père" />
              </div>
          ...
          <div className="modal-box" style={{ maxWidth:680 }} onClick={e => e.stopPropagation()}>
            ...
            {pdfParsing && (
              <div style={{ textAlign:'center', padding:40 }}>
                ...
                <div style={{ fontSize:12, color:'#9aa3b8' }}>Passerelle lit le document et extrait les événements</div>
              ...
              <>
                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:10 }}>
                  ...
                  <div style={{ display:'flex', gap:6 }}>
                    ...
                    <button className="btn btn-secondary" style={{ fontSize:10, padding:'4px 8px' }}
                      onClick={() => {
                        const none = {}
                        evtsImportes.forEach((_, i) => { none[i] = false })
                        setEvtsImportesChecked(none)
                      }}>
                      Tout décocher
                    </button>
                  </div>
                ...
                <div style={{ maxHeight:420, overflowY:'auto', marginBottom:14 }}>
                  {evtsImportes.map((evt, i) => {
                    const checked = evtsImportesChecked[i] === true
                      ...
                      <div key={i} style={{ display:'flex', gap:10, padding:'10px 12px', marginBottom:8, borderRadius:9, border:`2px solid ${checked ? cat.color : '#dde3f0'}`, background: checked ? cat.bg : '#f8f9fb', transition:'all .15s' }}>
                        ...
                        <div style={{ flex:1 }}>
                          ...
                          <div style={{ display:'flex', gap:6, flexWrap:'wrap', alignItems:'center' }}>
                            ...
                            {/* Date fin — affichée seulement si différente de date début (multi-jours) */}
                            {evt.date_fin && evt.date_fin.slice(0,10) !== evt.date_debut?.slice(0,10) && (
                              <input type="date"
                                style={{ fontSize:11, border:'1px solid #0891b2', borderRadius:6, padding:'3px 6px', background:'#e0f2fe' }}
                                value={evt.date_fin.slice(0,10)}
                                onChange={e => setEvtsImportes(prev => prev.map((ev,j) => j===i ? {...ev, date_fin: e.target.value + 'T' + (evt.date_fin?.slice(11) || '23:59:00')} : ev))}
                              />
                            )}
                            {/* Heure fin */}
                            <input type="time"
                          ...
                          {/* Notes / TISF */}
                          {evt.notes && (
                            <div style={{ marginTop:5, fontSize:10, fontStyle:'italic' }}>
                              {evt.notes.split(' · ').map((part, pi) => (
                                <span key={pi} style={{ color: part.includes('⚠️') ? '#c0392b' : '#5a6478', fontWeight: part.includes('⚠️') ? 700 : 400 }}>
                                  {pi > 0 ? ' · ' : ''}{part}
                                </span>
                              ))}
                            </div>
            ...
            {!editMode ? (
              <>
                ...
                {(() => {
                  ...
                  return (
                    <div style={{ background:'#eef1f8', borderRadius:9, padding:14, marginBottom:14, lineHeight:2, fontSize:12 }}>
                      ...
                      {/* Coordonnées de la structure relais */}
                      {selectedEvt.categorie === 'relais' && selectedEvt.relais_structure_id && relaisStructuresMap[selectedEvt.relais_structure_id] && (() => {
                        const s = relaisStructuresMap[selectedEvt.relais_structure_id]
                        return (
                          <div style={{ marginTop:10, padding:'8px 12px', background:'#e0f2fe', borderRadius:8, border:'1px solid #bae6fd' }}>
                            <div style={{ fontSize:11, fontWeight:700, color:'#0891b2', marginBottom:5 }}>🔄 Structure d'accueil</div>
                            <div style={{ fontSize:11, lineHeight:1.8 }}>
                              <div>📋 <strong>{s.nom}</strong></div>
                              {s.adresse && <div>📍 {s.adresse}</div>}
                              {s.telephone && <div>📞 <a href={`tel:${s.telephone}`} style={{ color:'#0891b2' }}>{s.telephone}</a></div>}
                              {s.email && <div>✉️ <a href={`mailto:${s.email}`} style={{ color:'#0891b2' }}>{s.email}</a></div>}
                              {s.notes && <div style={{ color:'#5a6478', fontStyle:'italic' }}>💬 {s.notes}</div>}
                            </div>
                          </div>
                        )
                      })()}
                ...
                <div className="modal-footer">
                  ...
                  {(selectedEvt.af_id === profile?.id || selectedEvt.cree_par === profile?.id) && (
                    <button className="btn btn-danger" onClick={() => deleteEvt(selectedEvt.id)}>🗑 Supprimer</button>
                  )}
                  {(selectedEvt.af_id === profile?.id || selectedEvt.cree_par === profile?.id || selectedEvt.participants_ids?.includes(profile?.id)) && (
      ...
      {/* ══════════════════════════════════════════════════════════════════════
          MODAL CRÉATION STRUCTURE RELAIS
      ══════════════════════════════════════════════════════════════════════ */}
      {showRelaisModal && (
        <div className="modal-overlay" onClick={() => setShowRelaisModal(false)}>
          <div className="modal-box" style={{ maxWidth:480 }} onClick={e => e.stopPropagation()}>
            <div className="modal-title">🔄 Nouvelle structure relais</div>
            <div className="form-grid-2">
              <div className="form-group col-span-2">
                <label className="form-label">Nom *</label>
                <input className="form-control" value={newRelais.nom}
                  onChange={e => setNewRelais(n => ({...n, nom: e.target.value}))}
                  placeholder="Ex: LVA Les Pins, Centre Anim. Gaillac, Jean Dupont..." />
              </div>
              <div className="form-group col-span-2">
                ...
                <div style={{ display:'flex', gap:5, flexWrap:'wrap' }}>
                  {[
                    ...
                    <button key={k} type="button"
                      onClick={() => setNewRelais(n => ({...n, type: k}))}
                      style={{ padding:'4px 9px', borderRadius:15, border:`1.5px solid ${newRelais.type === k ? '#0891b2' : '#dde3f0'}`, background: newRelais.type === k ? '#0891b2' : '#fff', color: newRelais.type === k ? '#fff' : '#5a6478', fontSize:11, cursor:'pointer' }}>
                      {l}
                    </button>
                  ))}
                </div>
              ...
              <div className="form-group col-span-2">
                ...
                <input className="form-control" value={newRelais.adresse}
                  onChange={e => setNewRelais(n => ({...n, adresse: e.target.value}))}
                  placeholder="Ex: 12 rue de la Paix, 81600 Gaillac" />
              </div>
              <div className="form-group">
                <label className="form-label">Téléphone</label>
                <input className="form-control" value={newRelais.telephone}
                  onChange={e => setNewRelais(n => ({...n, telephone: e.target.value}))}
                  placeholder="05 63 ..." />
              </div>
              <div className="form-group">
                <label className="form-label">Email</label>
                <input className="form-control" type="email" value={newRelais.email}
                  onChange={e => setNewRelais(n => ({...n, email: e.target.value}))}
                  placeholder="contact@..." />
              </div>
              <div className="form-group col-span-2">
                <label className="form-label">Notes</label>
                <textarea className="form-control" rows={2} value={newRelais.notes}
                  onChange={e => setNewRelais(n => ({...n, notes: e.target.value}))}
                  placeholder="Informations complémentaires..." style={{ resize:'vertical' }} />
              </div>
            </div>
            <div className="modal-footer">
              <button className="btn btn-secondary" onClick={() => setShowRelaisModal(false)}>Annuler</button>
              <button className="btn btn-primary" onClick={saveRelaisStructure}>💾 Enregistrer</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* ══════════════════════════════════════════════════════════════════════
          MODAL MODIFICATION CALENDRIER PDF
      ══════════════════════════════════════════════════════════════════════ */}
      {showModifModal && (
        <div className="modal-overlay" onClick={() => { if (!pdfModifParsing) { setShowModifModal(false); setEvtsModif([]) } }}>
          <div className="modal-box" style={{ maxWidth:720 }} onClick={e => e.stopPropagation()}>
            <div className="modal-title">📝 Modifier calendrier — Import PDF</div>

            {/* Zone upload */}
            {!pdfModifParsing && evtsModif.length === 0 && (
              <div
                style={{ border:'2px dashed #bae6fd', borderRadius:12, padding:28, textAlign:'center', cursor:'pointer', background:'#f0f9ff', marginBottom:14 }}
                onClick={() => document.getElementById('pdf-modif-input').click()}
                onDragOver={e => { e.preventDefault(); e.currentTarget.style.borderColor='#0891b2' }}
                onDragLeave={e => { e.currentTarget.style.borderColor='#bae6fd' }}
                onDrop={e => { e.preventDefault(); const f = e.dataTransfer.files[0]; if (f) parsePDFModif(f) }}
              >
                <div style={{ fontSize:36, marginBottom:8 }}>📝</div>
                <div style={{ fontSize:14, fontWeight:700, color:'#0891b2', marginBottom:4 }}>Calendrier MODIFIÉ</div>
                <div style={{ fontSize:12, color:'#9aa3b8', marginBottom:8 }}>Glissez votre PDF ici ou cliquez</div>
                <div style={{ fontSize:11, color:'#9aa3b8' }}>Le système comparera avec les événements existants et proposera les mises à jour</div>
                <input id="pdf-modif-input" type="file" accept=".pdf" style={{ display:'none' }}
                  onChange={e => { if (e.target.files[0]) parsePDFModif(e.target.files[0]) }} />
              </div>
            )}
            ...
            {pdfModifParsing && (
              <div style={{ textAlign:'center', padding:36 }}>
                <div style={{ fontSize:36, marginBottom:12 }}>🔍</div>
                <div style={{ fontSize:14, fontWeight:700, color:'#0891b2', marginBottom:6 }}>Analyse en cours...</div>
                <div style={{ fontSize:12, color:'#9aa3b8' }}>Passerelle compare le nouveau calendrier avec les événements existants</div>
                <div className="loading-spinner" style={{ marginTop:16 }}>⏳</div>
              </div>
            )}

            {/* Aperçu des modifications */}
            {evtsModif.length > 0 && !pdfModifParsing && (
              <>
                {/* Résumé */}
                {resumeModif && (
                  <div style={{ background:'#e0f2fe', borderRadius:9, padding:'10px 14px', marginBottom:12, fontSize:12, color:'#0891b2', fontWeight:600, border:'1px solid #bae6fd' }}>
                    📋 {resumeModif}
                  </div>
                )}
                ...
                <div style={{ display:'flex', gap:8, marginBottom:12 }}>
                  {[
                    { label: 'À supprimer', count: evtsModif.filter(e => e.action === 'supprimer').length, color:'#c0392b', bg:'#fdf0ee' },
                    { label: 'À modifier', count: evtsModif.filter(e => e.action === 'modifier' && e._evtExistant).length, color:'#d97706', bg:'#fef3e2' },
                    { label: 'À créer', count: evtsModif.filter(e => e.action === 'creer' || (e.action === 'modifier' && !e._evtExistant)).length, color:'#2e8b4a', bg:'#e6f5eb' },
                    { label: 'Inchangés', count: evtsModif.filter(e => e.action === 'inchange').length, color:'#9aa3b8', bg:'#eef1f8' },
                  ...
                  <div style={{ marginLeft:'auto', display:'flex', gap:6 }}>
                    <button className="btn btn-secondary" style={{ fontSize:10, padding:'4px 8px' }}
                      onClick={() => { const all={}; evtsModif.forEach((_,i) => all[i]=true); setEvtsModifChecked(all) }}>
                      Tout cocher
                    </button>
                    <button className="btn btn-secondary" style={{ fontSize:10, padding:'4px 8px' }}
                      onClick={() => { const none={}; evtsModif.forEach((_,i) => none[i]=false); setEvtsModifChecked(none) }}>
                      Tout décocher
                    </button>
                  </div>
                </div>
                ...
                <div style={{ maxHeight:420, overflowY:'auto', marginBottom:14 }}>
                  {evtsModif.map((evt, i) => {
                    const checked = evtsModifChecked[i] === true
                    const cat = CATEGORIES[evt.categorie] || CATEGORIES.vm
                    const isSuppr = evt.action === 'supprimer' && evt._evtExistant
                    const isModif = evt.action === 'modifier' && evt._evtExistant
                    const isCreate = evt.action === 'creer' || (evt.action === 'modifier' && !evt._evtExistant)
                    const isInchange = evt.action === 'inchange'

                    const actionColor = isSuppr ? '#c0392b' : isModif ? '#d97706' : isCreate ? '#2e8b4a' : '#9aa3b8'
                    const actionBg = isSuppr ? '#fdf0ee' : isModif ? '#fef3e2' : isCreate ? '#e6f5eb' : '#eef1f8'
                    const actionLabel = isSuppr ? '🗑️ Supprimer' : isModif ? '🔄 Modifier' : isCreate ? '➕ Créer' : '✓ Inchangé'

                    return (
                      <div key={i} style={{ display:'flex', gap:10, padding:'10px 12px', marginBottom:8, borderRadius:9, border:`2px solid ${checked ? cat.color : '#dde3f0'}`, background: checked ? '#fff' : '#f8f9fb', transition:'all .15s' }}>
                        {/* Checkbox */}
                        <div onClick={() => setEvtsModifChecked(prev => ({...prev, [i]: !checked}))}
                          style={{ width:20, height:20, borderRadius:5, border:`2px solid ${checked ? cat.color : '#9aa3b8'}`, background: checked ? cat.color : '#fff', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', flexShrink:0, marginTop:2 }}>
                          {checked && <span style={{ color:'#fff', fontSize:12, fontWeight:700 }}>✓</span>}
                        </div>
                        ...
                        <div style={{ flexShrink:0, marginTop:2 }}>
                          <span style={{ fontSize:10, padding:'2px 8px', borderRadius:10, background:actionBg, color:actionColor, fontWeight:700 }}>
                            {actionLabel}
                          </span>
                        </div>
                        ...
                        <div style={{ flex:1 }}>
                          ...
                          {isModif && evt._evtExistant && (
                            <div style={{ display:'grid', gridTemplateColumns:'1fr auto 1fr', gap:6, alignItems:'center', marginBottom:6 }}>
                              <div style={{ background:'#fde8e8', borderRadius:6, padding:'4px 8px', fontSize:11 }}>
                                <div style={{ fontSize:9, fontWeight:700, color:'#e05c5c', marginBottom:2 }}>AVANT</div>
                                <div>{new Date(evt._evtExistant.date_debut).toLocaleDateString('fr-FR', { weekday:'short', day:'numeric', month:'short' })}</div>
                                <div style={{ color:'#9aa3b8' }}>{new Date(evt._evtExistant.date_debut).toLocaleTimeString('fr-FR', { hour:'2-digit', minute:'2-digit' })} → {evt._evtExistant.date_fin ? new Date(evt._evtExistant.date_fin).toLocaleTimeString('fr-FR', { hour:'2-digit', minute:'2-digit' }) : ''}</div>
                              </div>
                              <span style={{ fontSize:18, color:'#d97706' }}>→</span>
                              <div style={{ background:'#e6f5eb', borderRadius:6, padding:'4px 8px', fontSize:11 }}>
                                <div style={{ fontSize:9, fontWeight:700, color:'#2e8b4a', marginBottom:2 }}>APRÈS</div>
                                <div style={{ fontWeight:700 }}>{new Date(evt.date_debut).toLocaleDateString('fr-FR', { weekday:'short', day:'numeric', month:'short' })}</div>
                                <div style={{ color:'#2e8b4a', fontWeight:600 }}>{new Date(evt.date_debut).toLocaleTimeString('fr-FR', { hour:'2-digit', minute:'2-digit' })} → {evt.date_fin ? new Date(evt.date_fin).toLocaleTimeString('fr-FR', { hour:'2-digit', minute:'2-digit' }) : ''}</div>
                              </div>
                            </div>
                          )}

                          {/* Création ou inchangé */}
                          {!isModif && (
                            <div style={{ fontSize:12, fontWeight:600, marginBottom:4 }}>
                              {new Date(evt.date_debut).toLocaleDateString('fr-FR', { weekday:'long', day:'numeric', month:'long' })}
                              {' · '}
                          ...
                          <div style={{ display:'flex', gap:6, alignItems:'center', flexWrap:'wrap' }}>
                            <span style={{ fontSize:11, padding:'2px 8px', borderRadius:10, background:cat.bg, color:cat.color, fontWeight:600 }}>{cat.label}</span>
                            {evt._enfantLabel && (
                              <span style={{ fontSize:10, padding:'2px 8px', borderRadius:10, background: evt.enfant_ids?.[0] ? (couleursEnfants[evt.enfant_ids[0]] || '#1a4b8f') : '#1a4b8f', color:'#fff', fontWeight:600 }}>
                                {evt._enfantLabel}
                              </span>
                            )}
                            {evt.lieu && <span style={{ fontSize:10, color:'#9aa3b8' }}>📍 {evt.lieu}</span>}
                          </div>
                ...
                {/* Résumé bas */}
                <div style={{ background:'#e6f5eb', borderRadius:8, padding:'8px 12px', marginBottom:12, fontSize:11, color:'#2e8b4a', fontWeight:600 }}>
                  ✅ {Object.values(evtsModifChecked).filter(v => v === true).length} action{Object.values(evtsModifChecked).filter(v => v === true).length > 1 ? 's' : ''} sélectionnée{Object.values(evtsModifChecked).filter(v => v === true).length > 1 ? 's' : ''}
                </div>
              </>
            )}

            <div className="modal-footer">
              <button className="btn btn-secondary" onClick={() => { setShowModifModal(false); setEvtsModif([]) }}>Annuler</button>
              {evtsModif.length > 0 && !pdfModifParsing && (
                <button className="btn btn-primary" onClick={saveEvtsModif}>
                  💾 Appliquer {Object.values(evtsModifChecked).filter(v => v === true).length} modification{Object.values(evtsModifChecked).filter(v => v === true).length > 1 ? 's' : ''}
                </button>
              )}
            </div>
          </div>
      ...
      {/* ══════════════════════════════════════════════════════════════════════
          MODAL SÉRIE — modifier/supprimer un ou toute la série
      ══════════════════════════════════════════════════════════════════════ */}
      {showSerieModal && (
        <div className="modal-overlay" onClick={() => setShowSerieModal(false)}>
          <div className="modal-box" style={{ maxWidth:420, textAlign:'center' }} onClick={e => e.stopPropagation()}>
            <div style={{ fontSize:32, marginBottom:12 }}>📅</div>
            <div className="modal-title" style={{ justifyContent:'center' }}>Cet événement fait partie d'une série</div>
            <p style={{ fontSize:13, color:'#5a6478', margin:'12px 0 24px' }}>
              Voulez-vous modifier uniquement cet événement ou tous les événements de la série ?
            </p>
            <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
              <button className="btn btn-primary"
                onClick={() => editSerieAction && editSerieAction(false)}>
                📝 Modifier uniquement cet événement
              </button>
              <button className="btn"
                style={{ background:'#e8eef8', color:'#1a4b8f', border:'1px solid #c4d4f5', fontFamily:'Sora,sans-serif', fontSize:13, padding:'10px', borderRadius:8, cursor:'pointer', fontWeight:600 }}
                onClick={() => editSerieAction && editSerieAction(true)}>
