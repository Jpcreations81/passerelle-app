...
export default function Agenda({ profile }) {
  ...
  async function saveEvt() {
    ...
    const relaisInfo = newEvt.categorie === 'relais' ? {
      ...
      relais_structure_id: newEvt.relais_structure_id || null,
            ...
            <div className="form-grid-2">
              ...
              {newEvt.categorie === 'relais' && (
                <div className="form-group col-span-2" style={{ background:'#e0f2fe', borderRadius:9, padding:12, border:'1px solid #bae6fd' }}>
                  ...
                  {/* Si AF → sélectionner dans profiles */}
                  {newEvt.relais_type === 'af' && (
                    <div>
                      <label className="form-label">Famille AF relais</label>
                      <select className="form-control"
                        value={newEvt.relais_structure_id || ''}
                        onChange={e => {
                          const af = Object.values(afProfiles).find(p => p.id === e.target.value) ||
                            collegues.find(c => c.id === e.target.value)
                          setNewEvt(n => ({ ...n, relais_structure_id: e.target.value, relais_nom_libre: af ? af.nom : '' }))
                        }}>
                        <option value="">— Sélectionner un AF —</option>
                        {collegues.filter(c => c.role === 'af' || !c.role).map(c => (
