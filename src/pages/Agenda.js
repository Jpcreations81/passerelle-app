    ...
    try {
      ...
      if (data.evenements && data.evenements.length > 0) {
        ...
        data.evenements.forEach(evt => {
          ...
          // Log pour debug : voir si enfants est bien chargé
          console.log('State enfants disponibles:', enfants.map(e => e.prenom + ' ' + e.nom))
          const ids = []
          ...
          console.log('Evt:', evt.titre, evt.date_debut, '| enfants_noms:', evt.enfants_noms, '| ids trouvés:', ids)
          ...
          if (ids.length > 1) {
            ...
            evtsExpanded.push({
              ...
              enfant_ids: ids,
              notes,
              _enfantLabel: ids.length === 1 ? (enfants.find(e => e.id === ids[0])?.prenom + ' ' + enfants.find(e => e.id === ids[0])?.nom) : ''
            })
          }
        })
                    ...
                    return (
                      <div key={i} style={{ display:'flex', gap:10, padding:'10px 12px', marginBottom:8, borderRadius:9, border:`2px solid ${checked ? cat.color : '#dde3f0'}`, background: checked ? cat.bg : '#f8f9fb', transition:'all .15s' }}>
                        ...
                        <div style={{ flex:1 }}>
                          ...
                          {!evt._enfantLabel && (
                            <div style={{ marginTop:5, fontSize:10, color:'#d97706', fontStyle:'italic' }}>⚠️ Enfant non reconnu — vérifiez le nom</div>
                          )}
                        </div>
                      </div>
                    )
