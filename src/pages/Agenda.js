...
export default function Agenda({ profile }) {
  ...
  // Demandes reçues : je suis valideur_id
  ...
  // Retours sur mes demandes envoyées (accepté ou refusé, non encore vus)
  const fetchMesRetours = useCallback(async () => {
    ...
    const { data } = await supabase
      ...
      .eq('vu_par_demandeur', false)
      .order('created_at', { ascending: false })
    if (data) setMesRetours(data)
  }, [profile])
  ...
  const fetchEnfants = useCallback(async () => {
    ...
    const { data: enfantsPrincipaux } = await supabase
      .from('enfants').select('id, nom, prenom').eq('af_principal_id', profile.id)
  ...
  async function accepterModif(demande) {
    ...
    await supabase.from('evenements_modifications')
      .update({ statut: 'accepte', vu_par_demandeur: false }).eq('id', demande.id)
    showToast('✅ Modification acceptée !')
    fetchDemandesModif(); fetchEvenements()
  }

  // ── Refuser une demande ───────────────────────────────────────────────────
  async function refuserModif(demande) {
    if (!refusMessage.trim()) { showToast("⚠️ Merci d'indiquer une raison de refus"); return }
    await supabase.from('evenements_modifications')
      .update({ statut: 'refuse', message_refus: refusMessage, vu_par_demandeur: false }).eq('id', demande.id)
  ...
  async function marquerLu(ids) {
    await supabase.from('evenements_modifications').update({ vu_par_demandeur: true }).in('id', ids)
    fetchMesRetours()
  }
  ...
  async function saveEvt() {
    ...
    const { error } = await supabase.from('evenements').insert({
      ...
      visible_ase: newEvt.categorie !== 'personnel', source: 'passerelle'
    })
  ...
  return (
    <div className="app-layout">
      ...
      ══════════════════════════════════════════════════════════════════════ */}
      {showValidationModal && (
        <div className="modal-overlay" onClick={() => { setShowValidationModal(false); setRefusMessage('') }}>
          <div className="modal-box" style={{ maxWidth:620 }} onClick={e => e.stopPropagation()}>
                  ...
                  return (
                    <div key={d.id} style={{ background:'#fef3e2', border:'1px solid #f5dca4', borderRadius:10, padding:14, marginBottom:12 }}>
                      ...
                      <input className="form-control" style={{ marginBottom:8, fontSize:11 }}
                        ...
                        value={refusMessage}
                        onChange={e => setRefusMessage(e.target.value)}
                      />
            ...
            {mesRetours.length > 0 && (
              <div style={{ marginBottom:14 }}>
                ...
                {mesRetours.map(d => (
                  <div key={d.id} style={{
                    background: d.statut === 'accepte' ? '#e6f5eb' : '#fdf0ee',
                    border: `1px solid ${d.statut === 'accepte' ? '#c4e8cc' : '#f5c4c4'}`,
                    borderRadius:10, padding:14, marginBottom:8
                  }}>
                    <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:6 }}>
                      <span style={{ fontSize:22 }}>{d.statut === 'accepte' ? '✅' : '❌'}</span>
                      <div>
                        <div style={{ fontSize:13, fontWeight:700 }}>
                          {d.statut === 'accepte' ? 'Modification acceptée' : 'Modification refusée'}
                          {' '}— <em>{d.evenement?.titre}</em>
                        </div>
                        <div style={{ fontSize:11, color:'#9aa3b8' }}>par {d.valideur?.prenom} {d.valideur?.nom}</div>
                      </div>
                    </div>
                    {d.statut === 'refuse' && d.message_refus && (
            ...
            <div className="modal-footer">
              <button className="btn btn-secondary" onClick={() => { setShowValidationModal(false); setRefusMessage('') }}>Fermer</button>
            </div>
          </div>
        </div>
      )}
