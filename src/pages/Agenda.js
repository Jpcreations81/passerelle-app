...
export default function Agenda({ profile }) {
  ...
  const [afProfiles, setAfProfiles] = useState({}) // id -> profile pour les relais
  ...
  const fetchEvenements = useCallback(async () => {
    ...
    // Fetch AF profiles for relais events (participants + owners)
    if (data) {
      const afIds = new Set()
      data.forEach(e => {
  ...
  async function saveEdit() {
    ...
    // Nettoyer les heures (supprimer les secondes si présentes)
    ...
    filtered.forEach(evt => {
      ...
      if (evt.enfant_ids && evt.enfant_ids.length > 0) {
        evt.enfant_ids.forEach((enfantId, idx) => {
          ...
          let titrePOV = enfantPrenom ? enfantPrenom + ' — ' + evt.titre.split(' — ')[0] : evt.titre
          if (evt.categorie === 'relais') {
            if (evt.af_id === profile?.id) {
              // Je suis l'AF principal — chercher le nom de la famille relais
              const participantId = evt.participants_ids?.[0]
              const afRelais = participantId ? afProfiles[participantId] : null
              const nomRelais = afRelais ? afRelais.nom : ''
              if (nomRelais) titrePOV = (enfantPrenom ? enfantPrenom + ' — ' : '') + 'Relais famille ' + nomRelais
            } else if (evt.participants_ids?.includes(profile?.id)) {
              // Je suis l'AF relais — chercher le nom de la famille principale
              const afPrincipal = afProfiles[evt.af_id]
              const nomPrincipal = afPrincipal ? afPrincipal.nom : ''
              if (nomPrincipal) titrePOV = (enfantPrenom ? enfantPrenom + ' — ' : '') + 'Relais famille ' + nomPrincipal
            }
          }
          expanded.push({
            ...evt,
      ...
      cells.push(
        ...
        >
          ...
          {evts.slice(0, 3).map((ev, ei) => {
            ...
            // Si le titre contient déjà le prénom, ne pas le répéter
            const label = (firstName && !titreFinal.includes(firstName)) ? firstName + ' — ' + titreFinal : titreFinal
            return (
                ...
                <div className="modal-footer">
                  ...
                  {(selectedEvt.af_id === profile?.id || selectedEvt.participants_ids?.includes(profile?.id)) && (
                    <button className="btn btn-primary" onClick={() => {
                      ...
                      // Récupérer les heures en heure locale Paris
                      ...
                      // Convertir date en locale Paris
                      const debDateParis = deb.toLocaleDateString('fr-CA', { timeZone:'Europe/Paris' }) // fr-CA = YYYY-MM-DD
