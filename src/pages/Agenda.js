...
import { supabase } from '../li// Agenda.js — v2026-05-25b — ouverture modal congé via URL params + PDF FicheConges
import React, { useState, useEffect, useCallback, useMemo } from 'react'
import { useNavigate, useLocation } from 'react-router-dom'
...
export default function Agenda({ profile }) {
  ...
  function buildTitreAuto(evt, enfantsList, couleursMap, profileData) {
    ...
    if (evt.categorie === 'vm' && evt.vm_presents?.length > 0) {
      ...
      const LABELS_TYPE = {
        af: 'famille', parrain: 'Parrain', tiers: 'Tiers',
                ...
                <div style={{ display:'flex', flexWrap:'wrap', gap:6, marginBottom:6 }}>
                  {(newEvt.dates_sup || []).map((d, i) => (
                    <div key={i} style={{ display:'flex', alignItems:'center', gap:4, padding:'3px 10px', borderRadius:15, background:'#e8eef8', border:'1px solid #c4d4f5', fontSize:11, fontWeight:600, color:'#1a4b8f' }}>
                      {(() => { const [y,m,day] = d.split('-'); return new Date(y, m-1, day).toLocaleDateString('fr-FR', { weekday:'short', day:'numeric', month:'short' }) })()}
