...
export default function Agenda({ profile }) {
  ...
  const fetchEnfants = useCallback(async () => {
    ...
    if (data) {
      setEnfants(data)
      // Couleurs par défaut par enfant
      const couleurs = {}
      const defaultColors = ['#e05c5c','#2e8b4a','#d97706','#6d4c9e','#0891b2','#1a4b8f']
      data.forEach((en, i) => {
        couleurs[en.id] = defaultColors[i % defaultColors.length]
      })
  ...
  function evtsDuJour(date) {
    ...
    // Expand events with multiple enfants into separate lines
    const expanded = []
    filtered.forEach(evt => {
      if (evt.enfant_ids && evt.enfant_ids.length > 1) {
        evt.enfant_ids.forEach(enfantId => {
          const enfant = enfants.find(e => e.id === enfantId)
          expanded.push({
            ...evt,
            _enfantId: enfantId,
            _enfantNom: enfant ? enfant.prenom + ' ' + enfant.nom : '',
            _couleur: couleursEnfants[enfantId] || (CATEGORIES[evt.categorie] || CATEGORIES.autre).color
          })
        })
      } else if (evt.enfant_ids && evt.enfant_ids.length === 1) {
        const enfantId = evt.enfant_ids[0]
        const enfant = enfants.find(e => e.id === enfantId)
        expanded.push({
          ...evt,
          _enfantId: enfantId,
        ...
        expanded.push({ ...evt, _couleur: (CATEGORIES[evt.categorie] || CATEGORIES.autre).color })
      }
    })
    return expanded
  }
      ...
      cells.push(
        ...
        >
          ...
          {evts.slice(0, 3).map((ev, ei) => {
            ...
            const label = ev._enfantNom ? ev._enfantNom.split(' ')[0] + ' — ' + ev.titre : ev.titre
            return (
              ...
              {evtsDuJour(d).length > 0 && (
                <div style={{ display:'flex', flexWrap:'wrap', gap:2, marginTop:4, justifyContent:'center' }}>
                  {evtsDuJour(d).map((ev, ei) => {
                    ...
                    const wLabel = ev._enfantNom ? ev._enfantNom.split(' ')[0] : ev.titre
        ...
        <div className="modal-overlay" onClick={() => setShowDetailModal(false)}>
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            ...
            {(() => {
              ...
              return (
                <div style={{ background:'#eef1f8', borderRadius:9, padding:14, marginBottom:14, lineHeight:1.8, fontSize:12 }}>
                  ...
                  <div style={{ marginTop:8 }}>
                    ...
                    {selectedEvt.source === 'ase_import' && <span style={{ marginLeft:6, padding:'3px 9px', borderRadius:10, background:'#e8eef8', color:'#1a4b8f', fontSize:10, fontWeight:600 }}>📥 Importé ASE</span>}
                  </div>
                </div>
              )
            })()}
            <div className="modal-footer">
              ...
              {selectedEvt.cree_par === profile?.id && (
                <button className="btn btn-danger" onClick={() => deleteEvt(selectedEvt.id)}>🗑 Supprimer</button>
              )}
              <button className="btn btn-primary" onClick={() => { setShowDetailModal(false); setNewEvt({ titre: selectedEvt.titre, categorie: selectedEvt.categorie, date_debut: selectedEvt.date_debut?.slice(0,10), heure_debut: selectedEvt.date_debut?.slice(11,16), date_fin: selectedEvt.date_fin?.slice(0,10), heure_fin: selectedEvt.date_fin?.slice(11,16), lieu: selectedEvt.lieu || '', notes: selectedEvt.notes || '' }); setShowModal(true) }}>✏️ Modifier</button>
            </div>
          </div>
        </div>
      )}
