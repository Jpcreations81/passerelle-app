...
export default function Agenda({ profile }) {
  ...
  // FIX: afProfiles est maintenant garanti d'être peuplé avant loading=false
  ...
  useEffect(() => {
    if (!profile) return
    // Lancer les 3 fetches en parallèle, mais setLoading(false) seulement quand tout est prêt
    Promise.all([
      fetchEvenements(),
      fetchPartages(),
      fetchCollegues(),
  ...
  // FIX: fetchEvenements ne met plus setLoading(false) lui-même
  // Il retourne une Promise et charge afProfiles de façon synchrone dans le même appel
  const fetchEvenements = useCallback(async () => {
    ...
    // Collecter tous les IDs enfants pour enrichir le state enfants
    const allEnfantIds = []
    data.forEach(e => {
      if (e.enfant_ids) e.enfant_ids.forEach(id => {
        if (!allEnfantIds.includes(id)) allEnfantIds.push(id)
      })
    })
    if (allEnfantIds.length > 0) {
      ...
      if (enf) {
        ...
        // FIX: Mettre à jour couleursEnfants pour les enfants relais aussi
        setCouleursEnfants(prev => {
          const updated = { ...prev }
          enf.forEach((en, i) => {
            if (!updated[en.id]) {
              updated[en.id] = DEFCOLORS[Object.keys(updated).length % DEFCOLORS.length]
            }
          })
          return updated
        })
    ...
    // FIX: Charger TOUS les profils AF nécessaires (af_id + participants_ids)
    // pour les événements relais, qu'on soit propriétaire OU participant
    const afIds = new Set()
    data.forEach(e => {
    ...
    if (afIds.size > 0) {
      const { data: profiles } = await supabase
        .from('profiles')
        .select('id, nom, prenom')
        .in('id', Array.from(afIds))
      if (profiles) {
        const profileMap = {}
        profiles.forEach(p => { profileMap[p.id] = p })
        console.log('AF Profiles chargés:', profileMap)
        // FIX: setAfProfiles est maintenant await-able via Promise.all dans useEffect
  ...
  const fetchEnfants = useCallback(async () => {
    ...
    // Chercher aussi les enfants via les événements relais (dont je suis participant)
    ...
    // Et les événements relais dont je suis propriétaire
    ...
    const ajouterIds = (evts) => {
      if (evts) evts.forEach(evt => {
        if (evt.enfant_ids) evt.enfant_ids.forEach(id => {
          if (!idsRelais.includes(id)) idsRelais.push(id)
        })
      })
    }
    ...
    // FIX: Couleurs stables — attribuer une couleur fixe par enfant basée sur son index
    const couleurs = {}
    tous.forEach((en, i) => {
      couleurs[en.id] = DEFCOLORS[i % DEFCOLORS.length]
    })
  ...
  // FIX: useMemo avec afProfiles, enfants et couleursEnfants comme dépendances
  // Garantit que evtsDuJour se recalcule quand les données async arrivent
      ...
      filtered.forEach(evt => {
        ...
        if (evt.enfant_ids && evt.enfant_ids.length > 0) {
          evt.enfant_ids.forEach((enfantId, idx) => {
            ...
            // FIX: couleur depuis couleursEnfants (stable, assignée à l'init)
            ...
            if (evt.categorie === 'relais') {
              if (evt.af_id === profile?.id) {
                // Je suis l'AF principal → afficher le nom de la famille relais
                const participantId = evt.participants_ids?.[0]
                const afRelais = participantId ? afProfiles[participantId] : null
                if (afRelais) {
                  titrePOV = (enfantPrenom ? enfantPrenom + ' — ' : '') + 'Relais fam. ' + afRelais.nom
                } else {
                  titrePOV = (enfantPrenom ? enfantPrenom + ' — ' : '') + evt.titre
                }
              } else if (evt.participants_ids?.includes(profile?.id)) {
                // Je suis l'AF relais → afficher le nom de la famille principale
                const afPrincipal = afProfiles[evt.af_id]
                if (afPrincipal) {
                  titrePOV = (enfantPrenom ? enfantPrenom + ' — ' : '') + 'Relais fam. ' + afPrincipal.nom
                } else {
                  titrePOV = (enfantPrenom ? enfantPrenom + ' — ' : '') + evt.titre
                }
              } else {
              ...
              // Autres catégories : prénom — titre
  ...
  // FIX: toutes les dépendances asynchrones ici → re-render quand elles arrivent
  }, [evenements, enfants, couleursEnfants, afProfiles, filtres, profile])

  // Wrapper pour garder l'API identique dans les render functions
  function evtsDuJour(date) {
    return buildEvtsDuJour(date)
  }
      ...
      cells.push(
        ...
        >
          ...
          {evts.slice(0, 3).map((ev, ei) => {
            ...
            const titreFinal = ev._titrePOV || ev.titre
            return (
              ...
              {evtsDuJour(d).length > 0 && (
                <div style={{ display:'flex', flexWrap:'wrap', gap:2, marginTop:4, justifyContent:'center' }}>
                  {evtsDuJour(d).map((ev, ei) => {
                    ...
                    const wLabel = ev._titrePOV || ev.titre
                    return (
                      <div key={ei} onClick={e => openEvt(ev, e)}
                        style={{ fontSize:9, padding:'2px 6px', borderRadius:3, background:wColor, color:'#fff', cursor:'pointer', fontWeight:500, maxWidth:90, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                        {wLabel}
                      </div>
                    )
                  })}
      ...
      <div style={{ background:'#fff', borderRadius:12, overflow:'hidden', boxShadow:'0 2px 12px rgba(26,75,143,.08)', border:'1px solid #dde3f0' }}>
        ...
        {evts.length === 0 ? (
          ...
          evts.map((ev, idx) => {
            ...
            const couleurBarre = ev._couleur || cat.color
            return (
              <div key={`${ev.id}-${idx}`} onClick={e => openEvt(ev, e)}
                style={{ display:'flex', gap:12, padding:'12px 16px', borderBottom:'1px solid #dde3f0', cursor:'pointer', borderLeft:`4px solid ${couleurBarre}`, transition:'all .15s' }}
              ...
              >
                ...
                <div style={{ flex:1 }}>
                  <div style={{ fontSize:13, fontWeight:600 }}>
                    {ev._enfantNom && (
                      <span style={{ fontSize:11, fontWeight:700, marginRight:6, padding:'1px 6px', borderRadius:8, background: ev._couleur || cat.color, color:'#fff' }}>
                        {ev._enfantNom.split(' ')[0]}
                      </span>
                    )}
                ...
                <div className="modal-footer">
                  ...
                  {(selectedEvt.af_id === profile?.id || selectedEvt.participants_ids?.includes(profile?.id)) && (
                    <button className="btn btn-primary" onClick={() => {
                      ...
                      const debParis = deb.toLocaleTimeString('fr-FR', { hour:'2-digit', minute:'2-digit', timeZone:'Europe/Paris' })
                      const finParis = fin.toLocaleTimeString('fr-FR', { hour:'2-digit', minute:'2-digit', timeZone:'Europe/Paris' })
                      const debDateParis = deb.toLocaleDateString('fr-CA', { timeZone:'Europe/Paris' })
                      const finDateParis = fin.toLocaleDateString('fr-CA', { timeZone:'Europe/Paris' })
                      setEditEvt({
                        titre: selectedEvt.titre,
                        categorie: selectedEvt.categorie,
                        date_debut: debDateParis,
                        heure_debut: debParis,
                        date_fin: finDateParis,
                        heure_fin: finParis,
                        lieu: selectedEvt.lieu || '',
                        notes: selectedEvt.notes || '',
                        message: ''
        ...
        <div className="modal-overlay" onClick={() => setShowPartageModal(false)}>
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            ...
            <div>
              ...
              {collègues.filter(c => !partages.some(p =>
