...
export default function Agenda({ profile }) {
  ...
  async function saveEvt() {
    ...
    if (!error) {
      ...
      setShowModal(false)
      setNewEvt({ titre:'', categorie:'vm', date_debut:'', heure_debut:'09:00', date_fin:'', heure_fin:'10:00', lieu:'', notes:'', enfantsSelectionnes:[], relais_type:'af', relais_structure_id:null, relais_nom_libre:'', vm_presents:[], complement_titre:'' })
      fetchEvenements()
    } else showToast('❌ Erreur : ' + error.message)
  }
  ...
  function buildTitreAuto(evt, enfantsList, couleursMap) {
    const enfantSelectionne = evt.enfantsSelectionnes?.length > 0
      ? enfantsList.find(e => e.id === evt.enfantsSelectionnes[0])
      : null
    const prenomInitiale = enfantSelectionne
      ? `${enfantSelectionne.prenom} ${enfantSelectionne.nom[0]}.`
      : ''
    ...
    const parts = [prenomInitiale, catLabel, complement].filter(Boolean)
