...
export default function Agenda({ profile }) {
  ...
  useEffect(() => { fetchEvenements(); fetchPartages(); fetchCollegues() }, [profile])
          ...
          {evts.slice(0, 3).map((ev, ei) => {
            ...
            return (
              <div key={ei} onClick={e => openEvt(ev, e)}
                style={{ fontSize:9, padding:'2px 5px', borderRadius:3, marginBottom:2, background:cat.color, color:'#fff', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis', cursor:'pointer', fontWeight:500 }}>
                {ev.titre}
              </div>
            )
          })}
              ...
              {evtsDuJour(d).length > 0 && (
                <div style={{ display:'flex', flexWrap:'wrap', gap:2, marginTop:4, justifyContent:'center' }}>
                  {evtsDuJour(d).map((ev, ei) => {
                    const cat = CATEGORIES[ev.categorie] || CATEGORIES.autre
                    return <div key={ei} onClick={e => openEvt(ev, e)} style={{ fontSize:9, padding:'2px 6px', borderRadius:3, background:cat.color, color:'#fff', cursor:'pointer', fontWeight:500, maxWidth:90, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{ev.titre}</div>
                  })}
                </div>
              )}
            </div>
