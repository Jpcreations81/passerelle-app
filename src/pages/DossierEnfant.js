...
export default function DossierEnfant({ profile }) {
  ...
  useEffect(() => {
    ...
    fetchCollegues()
    fetchJournal()
  }, [fetchEnfant, fetchCollegues, fetchJournal])

  // ── Sauvegarde ──────────────────────────────────────────────────────────────
  async function saveForm() {
    setSaving(true)
    // Exclure les relations (objets joints) et champs non-colonnes
    const { af_principal, referent, ...formData } = form
              ...
              <>
                <SectionCard icon="👤" title="État civil">
                  <FormGrid cols={3}>
                    ...
                    <Field label="N° de sécurité sociale" value={v('numero_secu')} onChange={F('numero_secu')} readOnly={!editMode} span={2} />
                    ...
                    {[
                      ...
                      <div key={doc.key} style={{ background:'#f4f6fb', borderRadius:10, padding:14, border:'1px solid #dde3f0' }}>
                        <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:12 }}>
                          ...
                          <span style={{ fontSize:12, fontWeight:600, color:'#1c2333' }}>{doc.label}</span>
                        </div>
                        {doc.fields.map(f => (
                          <div key={f.key} style={{ marginBottom:8 }}>
                            <label style={{ fontSize:10, fontWeight:600, color:'#5a6478', textTransform:'uppercase', letterSpacing:'.3px', display:'block', marginBottom:3 }}>{f.label}</label>
                            {editMode ? (
                              <input type={f.type || 'text'} value={v(f.key)} onChange={e => F(f.key)(e.target.value)}
                                style={{ width:'100%', padding:'7px 10px', border:'1.5px solid #dde3f0', borderRadius:7, fontFamily:'Sora,sans-serif', fontSize:12, background:'#fff', outline:'none' }} />
                            ) : (
                              <div style={{ padding:'7px 10px', background:'#fff', borderRadius:7, fontSize:12, color: v(f.key) ? '#1c2333' : '#9aa3b8', fontStyle: v(f.key) ? 'normal' : 'italic', border:'1px solid #dde3f0' }}>
                                {f.type === 'date' && v(f.key) ? new Date(v(f.key)).toLocaleDateString('fr-FR') : v(f.key) || '—'}
                              </div>
                            )}
                          </div>
                        ))}
                        <button onClick={() => showToast('📎 Upload document...')}
                          style={{ width:'100%', marginTop:4, padding:'6px', border:'1px dashed #c4d4f5', borderRadius:7, background:'#e8eef8', color:'#1a4b8f', fontSize:11, cursor:'pointer', fontFamily:'Sora,sans-serif' }}>
                          📎 Ajouter scan
                        </button>
                      </div>
                    ))}
                    {[
                      ...
                      <div key={doc.key} style={{ background:'#f4f6fb', borderRadius:10, padding:14, border:'1px solid #dde3f0' }}>
                        ...
                        <div style={{ marginBottom:8 }}>
                          <label style={{ fontSize:10, fontWeight:600, color:'#5a6478', textTransform:'uppercase', letterSpacing:'.3px', display:'block', marginBottom:3 }}>Statut</label>
                          {editMode ? (
                            <select value={v(doc.statusKey)} onChange={e => F(doc.statusKey)(e.target.value)}
                              style={{ width:'100%', padding:'7px 10px', border:'1.5px solid #dde3f0', borderRadius:7, fontFamily:'Sora,sans-serif', fontSize:12, background:'#fff', outline:'none' }}>
                              <option value="">—</option>
                              {doc.options.map(o => <option key={o} value={o}>{o}</option>)}
                            </select>
                          ) : (
                            <div style={{ padding:'7px 10px', background:'#fff', borderRadius:7, fontSize:12, color: v(doc.statusKey) ? '#1c2333' : '#9aa3b8', fontStyle: v(doc.statusKey) ? 'normal' : 'italic', border:'1px solid #dde3f0' }}>
                              {v(doc.statusKey) || '—'}
                            </div>
                          )}
                        </div>
