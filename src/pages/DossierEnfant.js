...
export default function DossierEnfant({ profile }) {
  ...
  async function addFratrieNew(parentCommun) {
    ...
    const af_id = newFratrie.lieu_type === 'af' && newFratrie.meme_af
      ? form.af_principal_id
      : null

    // Copier les parents selon le choix
    const dataParents = {}
    if (parentCommun === 'les_deux' || parentCommun === true) {
      dataParents.pere_nom = form.pere_nom; dataParents.pere_prenom = form.pere_prenom
      dataParents.pere_tel = form.pere_tel; dataParents.pere_adresse = form.pere_adresse
      dataParents.pere_droit_visite = form.pere_droit_visite; dataParents.pere_droits = form.pere_droits
      dataParents.mere_nom = form.mere_nom; dataParents.mere_prenom = form.mere_prenom
      dataParents.mere_tel = form.mere_tel; dataParents.mere_adresse = form.mere_adresse
      dataParents.mere_droit_visite = form.mere_droit_visite; dataParents.mere_droits = form.mere_droits
    ...
    const msgs = {
      'les_deux': '✅ Dossier créé — père et mère copiés !',
      'pere': '✅ Dossier créé — père copié (même père)',
      'mere': '✅ Dossier créé — mère copiée (même mère)',
    }
    ...
    const fraternieMembres = (form.fratrie || []).filter(f => f.enfant_id).map(f => ({
      enfant_id: f.enfant_id, prenom: f.prenom, nom: f.nom, ddn: f.ddn, sexe: f.sexe, meme_af: f.meme_af
    }))

    await supabase.from('enfants').update({
      fratrie: [...fraternieMembres, moiMembre]
    }).eq('id', nouveauDossier.id)
    ...
    setFratrieMode('question')
    setNewFratrie({ prenom:'', nom:'', ddn:'', sexe:'M', meme_af:false, type_placement:'non_place', lieu_type:'', lieu_nom:'', memes_parents: null })
    const msg = memesParents ? '✅ Dossier créé avec parents copiés !' : '✅ Dossier créé — pensez à renseigner les parents'
    showToast(msg)
  }
    ...
    // Synchroniser vers tous les membres qui ont un enfant_id
    const moiMembre = {
      enfant_id: id,
      prenom: form.prenom,
      nom: form.nom,
    ...
    // Ajouter moi dans le nouveau dossier
    await supabase.from('enfants').update({
      fratrie: [...(form.fratrie || []).map(f => ({
        enfant_id: f.enfant_id, prenom: f.prenom, nom: f.nom, ddn: f.ddn, sexe: f.sexe, meme_af: f.meme_af
      })).filter(f => f.enfant_id), moiMembre]
    }).eq('id', nouveauDossier.id)

    // Synchroniser vers les autres membres existants
    for (const membre of (form.fratrie || [])) {
      if (membre.enfant_id) {
        await syncFratrieMiroir(membre.enfant_id, newMembre)
        await syncFratrieMiroir(nouveauDossier.id, membre)
    ...
    setShowFratrieModal(false)
    setFratrieMode('question')
    setNewFratrie({ prenom:'', nom:'', ddn:'', sexe:'M', meme_af:false, type_placement:'non_place', lieu_type:'', lieu_nom:'' })
    showToast(`✅ Dossier créé et fratrie synchronisée !`)
  }
            ...
            {onglet === 'famille' && (
              <>
                {['pere', 'mere'].map(parent => {
                  const masque = form[`${parent}_masque`]
                  return (
                    <SectionCard key={parent} icon={parent === 'pere' ? '👨' : '👩'}
                      title={
                        <div style={{ display:'flex', alignItems:'center', gap:10, width:'100%' }}>
                          <span>{parent === 'pere' ? 'Père' : 'Mère'}</span>
                          {masque && <span style={{ padding:'2px 8px', borderRadius:10, background:'#fef3e2', color:'#d97706', fontSize:11, fontWeight:600 }}>Masqué</span>}
                          {editMode && (
                            <button onClick={() => F(`${parent}_masque`)(!masque)}
                              style={{ marginLeft:'auto', padding:'3px 10px', borderRadius:8, border:'1px solid #dde3f0', background: masque ? '#e6f5eb' : '#fdf0ee', color: masque ? '#2e8b4a' : '#c0392b', fontSize:11, cursor:'pointer' }}>
                      ...
                      {masque ? (
                        <div style={{ color:'#9aa3b8', fontStyle:'italic', fontSize:13 }}>
                          {parent === 'pere' ? "Père non renseigné (famille monoparentale ou orphelin)" : "Mère non renseignée (famille monoparentale ou orphelin)"}
                        </div>
                      ) : (
                        <FormGrid cols={3}>
                          ...
                          <Field label="Droit de visite" value={v(`${parent}_droit_visite`)} onChange={F(`${parent}_droit_visite`)} readOnly={!editMode}
                            options={['Visite médiatisée','Visite libre','Mixte (médiatisé + temps libre)','Aucun droit','Suspendu']} />
                        </FormGrid>
                      )}
                    </SectionCard>
                  )
                    ...
                    <div key={i} style={{ display:'flex', alignItems:'center', gap:12, padding:'10px 14px', background:'#f4f6fb', borderRadius:8, marginBottom:8, border:'1px solid #dde3f0' }}>
                      ...
                      <div style={{ flex:1 }}>
                        ...
                        <div style={{ fontSize:11, color:'#9aa3b8' }}>
                          {f.ddn && (() => {
                            const [y,m,d] = f.ddn.split('-')
                            const date = new Date(y, m-1, d)
                            return `${calcAge(f.ddn)} · Né(e) le ${date.toLocaleDateString('fr-FR')}`
                          })()}
                        </div>
                      </div>
                      <span style={{ padding:'3px 10px', borderRadius:10, fontSize:11, fontWeight:600,
                        ...
                        {f.lieu_label || (f.meme_af ? "Même famille d'accueil" : "Autre famille d'accueil")}
                      </span>
                      {f.enfant_id && (
