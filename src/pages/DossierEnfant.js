...
export default function DossierEnfant({ profile }) {
  ...
  const [fratrieMode, setFratrieMode] = useState('question') // 'question', 'search', 'create'
  const [newFratrie, setNewFratrie] = useState({ prenom:'', nom:'', ddn:'', sexe:'M', meme_af:false, type_placement:'non_place', lieu_type:'', lieu_nom:'' })
  ...
  async function addFratrieNew() {
    ...
    const { data: nouveauDossier, error } = await supabase.from('enfants').insert({
      ...
      referent_id: enfant?.referent_id || null,
      territoire: enfant?.territoire || profile?.territoire,
    }).select().single()

    if (error) { showToast('❌ Erreur création dossier : ' + error.message); return }

    // Ajouter comme membre de la fratrie avec enfant_id
    const newMembre = {
    ...
    setShowFratrieModal(false)
    setFratrieMode('question')
    setNewFratrie({ prenom:'', nom:'', ddn:'', sexe:'M', meme_af:false })
    showToast('✅ Membre ajouté et synchronisé !')
  }
