...
export default function DossierEnfant({ profile }) {
  ...
  async function addFratrieFromBase(enfant) {
    ...
    // Sauvegarder immédiatement en base pour l'enfant courant
    await supabase.from('enfants').update({ fratrie: newFratrieList }).eq('id', id)
    setForm(f => ({ ...f, fratrie: newFratrieList }))

    // Moi en tant que membre pour les autres
    const moiMembre = {
      enfant_id: id,
      prenom: form.prenom,
      nom: form.nom,
    ...
    // Ajout miroir : ajouter l'enfant courant dans la fratrie de l'enfant ajouté
    await syncFratrieMiroir(enfant.id, moiMembre)

    // Synchroniser aussi les autres membres existants de la fratrie
    for (const membre of (form.fratrie || [])) {
      if (membre.enfant_id) {
        await syncFratrieMiroir(membre.enfant_id, newMembre)
        await syncFratrieMiroir(enfant.id, membre)
      }
    }
  ...
  function addFratrieNew() {
    ...
    const lieu_label = newFratrie.lieu_type === 'af'
      ? (newFratrie.meme_af ? "Même famille d'accueil" : "Autre famille d'accueil")
      : newFratrie.lieu_nom || newFratrie.lieu_type || ''
    setForm(f => ({ ...f, fratrie: [...(f.fratrie || []), {
      prenom: newFratrie.prenom,
      nom: newFratrie.nom,
      ddn: newFratrie.ddn,
      sexe: newFratrie.sexe,
      meme_af: newFratrie.meme_af,
      type_placement: newFratrie.type_placement,
      lieu_type: newFratrie.lieu_type,
      lieu_label,
    }] }))
    setShowFratrieModal(false)
    setFratrieMode('question')
