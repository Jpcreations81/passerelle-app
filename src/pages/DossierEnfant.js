...
export default function DossierEnfant({ profile }) {
  ...
  const isReferent = ['referent','encadrant','rtase','admin'].includes(profile?.role) && !nonPlace
  const isAF = profile?.role === 'af'
  const canEdit = isReferent || (isAF && (enfant?.af_principal_id === profile?.id || nonPlace))
