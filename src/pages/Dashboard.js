...
export default function Dashboard({ profile, session }) {
  ...
  const [demandesModif, setDemandesModif] = useState([])   // modifs à valider (je suis propriétaire)
  const [mesRetours, setMesRetours] = useState([])          // réponses sur mes demandes envoyées
  ...
  async function fetchMesRetours() {
    ...
    const { data } = await supabase
      .from('evenements_modifications')
      .select('id, statut, evenement:evenement_id(titre)')
  ...
  const nbAccepte = mesRetours.filter(d => d.statut === 'accepte').length
  const nbRefuse = mesRetours.filter(d => d.statut === 'refuse').length
        ...
        <div className="page-content">
          ...
          <div className="card">
            ...
            <div className="card-body">
              ...
              {/* ── Alerte : demandes de modif agenda à valider ── */}
              ...
              {/* ── Aucune alerte ── */}
              {profile?.role === 'af' && enfants.length === 0 && demandesModif.length === 0 && mesRetours.length === 0 && (
