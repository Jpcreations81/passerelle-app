  ...
  async function loadFiche() {
    ...
    if (evts) {
      evts.forEach(evt => {
        ...
        // Récupérer tous les jours du relais dans le mois
        ...
        while (cur <= dFin) {
          ...
          if (p[key]) {
            if (isFirst) {
              ...
              p[key].motif = 'Départ en relais chez ' + (evt.notes ? evt.notes.split('.')[0] : 'AF Relais')
            } else if (fmt(cur) === fmt(dFin)) {
              // Dernier jour : coché, heure d'arrivée
              p[key].present = true
              p[key].heure_arrivee = fmtHeure(evt.date_fin)
              p[key].motif = 'Retour de relais'
            } else {
      ...
      // Charger aussi les VM et réunions pour les heures
      const { data: autres } = await supabase
        .from('evenements')
        .select('*')
        .eq('af_id', profile.id)
        .in('categorie', ['vm', 'ase', 'medical', 'scolaire'])
        .gte('date_debut', debut.toISOString())
        .lte('date_debut', fin.toISOString())

      if (autres) {
        autres.forEach(evt => {
          const key = fmt(new Date(evt.date_debut))
          if (p[key] && !p[key].motif) {
            p[key].heure_depart = fmtHeure(evt.date_debut)
            p[key].heure_arrivee = fmtHeure(evt.date_fin)
          }
        })
      ...
      <div className="main-content">
        ...
        <style>{`
          @media print {
            ...
            .fiche-print { border:none!important; box-shadow:none!important; border-radius:0!important; }
          }
        `}</style>
        ...
        <div className="page-content">
          ...
          {selectedEnfant && (
            <div className="fiche-print" style={{ background:'#fff', border:'2px solid #1a4b8f', borderRadius:10, overflow:'hidden', boxShadow:'0 4px 20px rgba(26,75,143,.1)' }}>
              ...
              {/* EN-TÊTE */}
              <div style={{ background:'#dbeafe', borderBottom:'2px solid #1a4b8f', padding:'12px 18px', display:'flex', alignItems:'center', gap:16, flexWrap:'wrap' }}>
                ...
                <table style={{ width:'100%', borderCollapse:'collapse', fontSize:11 }}>
                  ...
                  <tbody>
                    {days.map((d, i) => {
                      ...
                      const isBlue = dim || fe
                      return (
                        <tr key={i} style={{ background: isBlue ? '#dbeafe' : p.present ? '#fff' : '#fff9e6' }}>
